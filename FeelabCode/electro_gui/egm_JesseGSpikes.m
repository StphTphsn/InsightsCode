...
while ii <= length(param.filerange)
    ...
    if rep == 1
        ...
        templates{end+1} = kernel{1};
        templates{end+1} = kernel{2};
        rel_times(end+1,:) = [tm(1) inf];
        rel_times(end+1,:) = [inf tm(2)];
    end
    ...
    for c = 1:length(templates)
        ...
        [log_txt log_but] = UpdateLog(log_txt,log_but,log_str);
        energ = mean(templates{c}.^2);
        if c == 1
            errs = dbaseLSQ(templates{c},interpdata)/energ;
    ...
    if rep == 0
        ...
        [old_kernel2 old_tm dummy_interpdata dummy_param] = getKernel(dbase,handles,param,i);
        for c = 1:length(old_kernel2)
            if length(old_kernel2{c})>length(old_kernel{c})
                old_kernel2{c} = old_kernel2{c}(1:length(old_kernel{c}));
            end
            if length(old_kernel2{c})<length(old_kernel{c})
                old_kernel2{c}(end+1:length(old_kernel{c})) = 0;
            end
        end
        old_kernel = old_kernel2;
    end
end
