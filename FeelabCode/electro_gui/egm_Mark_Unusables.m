function handles = egm_Mark_Unusables(handles)

answer = inputdlg({'Sequences of syllable labels to consider','Maximum syllable separation (sec)','Boolean property name'},'Mark files',1,{'{}','0.2','bUnusable'});
if isempty(answer)
    return
end
...
for c = 1:handles.TotalFileNumber
    if ~isempty(handles.SegmentTimes{c})
        ...
        son = handles.SegmentTimes{c}(f,1);
        soff = handles.SegmentTimes{c}(f,2);
        titl = handles.SegmentTitles{c}(f);
        stitl = '';
        for j = 1:length(titl)
    ...
    handles.Properties.Values{c}{end+1} = ~should_mark;%this is wehre you reverse logic for bunusable
