function handles = egm_QuickTime_animation(handles)


answer = inputdlg({'Height (pixels)','Width (pixels/sec)','JPEG quality (0 to 1)','Frame rate (fps)','Progress bar width (pixels)'},'Quicktime parameters',1,...
    {'100','500','0.5','60','5'});
if isempty(answer)
    return
end
im.height = str2num(answer{1});
im.width = str2num(answer{2});
im.quality = str2num(answer{3});
im.framerate = str2num(answer{4});
im.bar_width = str2num(answer{5});


[pathstr,name,ext,versn] = fileparts(get(handles.text_FileName,'string'));
[file, path] = uiputfile([handles.DefaultDirectory '\' name '.mov'],'Save movie');
if ~isstr(file)
    return
end

% START MOVIE
MakeQTMovie('start',[path file]);
MakeQTMovie('framerate',im.framerate);
MakeQTMovie('quality',im.quality);
...
pos(3) = round(im.width*(xl(2)-xl(1)));
pos(4) = im.height;
...
AAquick_sonogram(gca,handles.sound(xlp(1):xlp(2)),handles.fs);
...
print('-djpeg',['-f' num2str(fig)],[path 'tempfile.jpg']);
delete(fig);

img = double(imread([path 'tempfile.jpg']))/255;
img = img(:,1:end-1,:);
spec = zeros(pos(4),pos(3),3);
for i = 1:3
    spec(:,:,i) = resizem(img(:,:,i),[pos(4) pos(3)]);
end

delete([path 'tempfile.jpg']);


% ADD FRAMES TO MOVIE
stp = 1/im.framerate*im.width*handles.SoundSpeed;
tms = round(1:stp:size(spec,2));
for c = 1:length(tms)
    m = spec;
    if c > 1
        for i = 1:3
            m(:,max([1 tms(c)-im.bar_width+1]):tms(c),i) = handles.ProgressBarColor(i);
        end
    end
    MakeQTMovie('addmatrix',m);
end
MakeQTMovie('addmatrix',spec);


% ADD SOUND TO MOVIE
realfs = handles.fs;
snd = handles.sound(xlp(1):xlp(2));
indx = round(linspace(1,length(snd),length(snd)));
snd = snd(indx);
MakeQTMovie('addsound',snd,round(realfs*handles.SoundSpeed));
...
function p = AAquick_sonogram(ax,wv,fs)
