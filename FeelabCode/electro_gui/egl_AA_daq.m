...
if loaddata == 1
    [data info] = daq_readDatafile_forEG(filename,1,[]); % load all samples
else % loaddata==0
    [data info] = daq_readDatafile_forEG(filename,1,0); % load no samples
end

%-- see description below for the structure of info
fs = info.fs;
...
if(trigFileFormat == -4)
    ...
    fseek(fid, 0, 'eof');
    fidScan = ftell(fid); % get file position indicator
    while(true)
        fidScan = fidScan - 1;
        fseek(fid, fidScan, 'bof'); % beginning of file
    ...
    while(~feof(fid))
        ...
        temp = fread(fid, strLen, 'char'); %%% often cause of the error because strLen is not integer
