...
% for row = [15 14 13 11 12]; %1:13%[5 7:10]%6 5
%     try 
...
load(['rec_allthefeatures_' num2str(row) '.mat']);
...
%RampRamp = create_ramp(AllLabelsCDF(FeatureInd.TimeFromOnset,:)); 
% modifying ramp to represent real time course
...
%     RealTime(beg_ind_syl(syl):end_ind_syl(syl)) = (1:length(beg_ind_syl(syl):end_ind_syl(syl)))/length_biggest_syllable;
% end
% RealTime(RealTime==-2) =0;  
% figure;plot(RealTime)

figure;imagesc(RampRamp);colorbar;
drawnow
...
for i =1:length(attempts)
    ...
    switch attempt
        ...
        case 'specplusramp'
            ...
            ToTSNE2 = [ToTsneAll([FeatureInd.Spectrogram],:); med*.1*RampRamp];
            IndKeep = 1:size(ToTSNE2,1);  
    end
    tic
...
save(['lastFee3_' attempt '_row' num2str(row)], 'Spectro','tSNE_Coord','selected', 'FeatureInd')
